...
public abstract class IXEditorPage extends FormPage { // implements
	 ...
	 *            the parent editor
	 */
	public abstract void initializeWorkflowModel(WorkflowModel amodel);
	...
	public Composite createSection(IManagedForm mform, String title,
			String desc,int iSectionStyle, int numColumns, GridData aGridData,ImageDescriptor aImageDesc) {
		FormToolkit toolkit = mform.getToolkit();

		Composite client = createSection(mform, title, desc,iSectionStyle, numColumns,aGridData);
	...
	public void createFormInput(Composite parent, FormToolkit toolkit,
			String sLabel, String sPropertyName, ModelObject modelObject) {
		// Create Label
	...
	public void createTableInput(Composite parent, FormToolkit toolkit,
			String sPropertyName, ModelObject modelObject,final Image itemImage) {
		...
		setGridData(composite, 280, 195, GridData.FILL, true,
				GridData.FILL, true);
		...
		button = toolkit.createButton(compositeButtons, "Edit",
				SWT.PUSH);
		button.addSelectionListener(new SelectionAdapter() {
			public void widgetSelected(SelectionEvent e) {
